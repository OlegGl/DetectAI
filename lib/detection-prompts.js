...
// Compact prompt (~300 tokens) for local models. Ollama has no cheap prompt
// cache, so the full VERMILLION essay would be re-prefilled on every chunk,
// dominating latency on a local model. This keeps the signal names, the label
// enum, the calibration bands, and the exact output schema.
const SYSTEM_PROMPT_COMPACT = `You are a forensic linguist detecting AI-generated text. Classify each segment as human or AI written.

AI signals: uniform sentence length, formulaic transitions (Moreover/Furthermore/Additionally/In conclusion), over-hedging (might/could/may), no personal voice or lived experience, rigid same-shaped paragraphs, heavy nominalization, passive voice, vague pronoun referents, mechanical em-dashes.
Human signals: high sentence-length variance, idiosyncratic word choice, first-person specifics, anecdotes, emotional grounding, deliberate short emphatic sentences, small grammatical quirks.

Confidence: 0.0–0.35 strongly human, 0.35–0.55 ambiguous, 0.55–0.80 probably AI, 0.80–1.0 strongly AI.
Use label "insufficient" for text under ~80 characters and "skip" for code or quotations.

Return ONLY valid JSON, no prose, exactly this shape:
{"segments":[{"id":"<id>","label":"ai|human|ambiguous|skip|insufficient","confidence":0.0,"reasoning":"one sentence","signals":["formulaic_transitions"]}]}
Include every provided segment ID exactly once. Do not invent IDs.`;

// Streaming variant: emit ONE JSON object per line (JSONL), no array wrapper and
// NO reasoning field — so the extension can parse each line the instant it
// arrives and paint that outline, and so the model generates far fewer output
// tokens (reasoning is the biggest cost). Reasoning is shown lazily as the
// detected signals instead. Must NOT be combined with format:"json".
const SYSTEM_PROMPT_STREAM = `You are a forensic linguist detecting AI-generated text. Classify each segment as human or AI written.
...
Output exactly one JSON object per line, one per segment, in the order given. No array, no prose, no markdown. Emit each object as soon as you finish it.
Each line MUST be: {"id":"<id>","label":"ai|human|ambiguous|skip|insufficient","confidence":0.0,"signals":["formulaic_transitions"]}
Use the exact id given for each segment. Include every segment exactly once.`;

// Bump when any prompt/label/signal semantics change — invalidates the cache.
 ...
 * - 'ollama': compact non-streaming prompt (single JSON object).
...
export function buildSystemPrompt(mode) {
  ...
  if (mode === 'ollama') return SYSTEM_PROMPT_COMPACT;
...
export function normalizeSegment(seg, id) {
  const label = VALID_LABELS.has(seg.label) ? seg.label : 'ambiguous';
  const confidence =
  ...
  const reasoning =
    typeof seg.reasoning === 'string' && seg.reasoning.trim().length > 0
      ? seg.reasoning.trim()
      : '';
  // Keep any string signals the model emits (local models use their own
  // vocabulary, and signals are the tooltip's explanation on the streaming
  // path where reasoning is omitted). Sanitize + cap.
