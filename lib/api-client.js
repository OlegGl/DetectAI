...
async function cacheModelId(settings) {
  if (settings.apiBackend === 'ollama') {
    ...
    return 'ollama:' + (await resolveOllamaModel(url, settings.ollamaModel || 'qwen2.5:3b'));
  }
...
export async function preloadOllama(settings) {
  try {
    ...
    const model = await resolveOllamaModel(ollamaUrl, settings.ollamaModel || 'qwen2.5:3b');
...
async function callOllamaAPI(paragraphsChunk, settings, onDebug, onPartial) {
  ...
  // Anything the streaming parse missed (malformed line, reordered ids) — fall
  // back to the tolerant whole-response parser so every id still gets a verdict.
  const missing = paragraphsChunk.filter((p) => !collected.has(p.id));
  if (missing.length > 0) {
