...
export async function detectTextBatch(paragraphs, settings, hooks = {}) {
  ...
  // Chunk only the misses. Content.js batches cap at 10 ids per message, so
  // this is one chunk per message; kept at 10 so the num_ctx sizing holds.
  const chunkSize = 10;
  const chunks = [];
...
// Sizing: num_ctx stays at 8192 — empirically, both num_ctx 4096 and a raised
// num_batch wedged the macOS runner (requests hung past the headers timeout),
// while 8192 is proven stable. Output is one short JSONL line per segment
// (~40 tokens × 10), so num_predict 768 bounds runaway generation without
// truncation risk.
const OLLAMA_OPTIONS = { temperature: 0, top_p: 0.9, num_predict: 768, num_ctx: 8192 };
